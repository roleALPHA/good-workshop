...
export async function loadMembers(): Promise<ActionResult<MemberRow[]>> {
  ...
  if (actor.tenantRole !== 'admin') return fail('Nur Tenant-Admins sehen die Mitgliederliste.')
...
export async function inviteMemberAction(raw: {
  ...
  const parsed = z
    ...
    .safeParse(raw)
  if (!parsed.success) return fail('Bitte E-Mail-Adresse und Rolle prüfen.')
...
function toResult<T>(error: unknown): ActionResult<T> {
  // A MemberError is a sentence written for the person reading it -- "that is
  // the last active admin" -- and passing it through is the whole point of
  // having the type.
  if (error instanceof MemberError) return fail(error.message)
