 ...
 *
 * The vocabulary is authored at runtime, so a facet group keyed `q` or `time`
 * is a migration away from existing -- and it would be skipped in silence: its
 * chips would render from the facet list, change the address, and narrow
 * nothing. Exported so `filters.tsx` can carry these through its form, which is
 * the other half of the same problem.
 */
export const RESERVED: readonly string[] = [GROUP, TIME, SEARCH, CURSOR]
