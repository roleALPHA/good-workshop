...
import { ok } from '@/server/mcp/respond'
...
/**
 * The Discover catalogue, for a model.
 *
 * Whole documents rather than one tool per field, for the reason apply_agenda
 * exists: twenty dependent calls is where a model loses the thread, and a
 * half-written method is worse than an unwritten one.
 *
 * Authoring and publishing are separate scopes on purpose. A token handed to a
 * model for bulk import should be able to fill the catalogue and unable to make
 * any of it live.
 *
 * The workspaces are next door in ./mcp-tenant-tools.ts.
 */
export function registerOperatorCatalogTools(server: McpServer, actor: OperatorActor): void {
  const db = operatorDb()
