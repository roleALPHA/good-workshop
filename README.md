...
`GW_VERSION` names when the stack first comes up. The current release is `v0.4.15`, published as the image tag `0.4.15` (without the `v`); there is
nothing to build.
...
```bash
docker buildx imagetools inspect ghcr.io/rolealpha/good-workshop:0.4.15 --format '{{ json .SBOM }}'
```
