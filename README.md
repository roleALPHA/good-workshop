...
release (currently `v0.8.8`). Every release is also published under its own tag, without the
`v` (`0.8.8`) — set that instead to stay on one version until you decide to update. There is
nothing to build.
...
GW_VERSION=latest                         # the newest stable release, or e.g. 0.8.8 to pin one
```
...
| `GW_VERSION`                                             | yes          | Image tag: `latest` for the newest stable release, or a release such as `0.8.8` to stay on it. Shown in the page footer and by `/api/health`. |
...
```bash
docker buildx imagetools inspect ghcr.io/rolealpha/good-workshop:0.8.8 --format '{{ json .SBOM }}'
```
