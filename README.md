...
`GW_VERSION` names when the stack first comes up. The current release is `v0.4.1`; there is
nothing to build.
...
GW_VERSION=v0.4.1                         # the release to run, never a moving tag
```
...
```bash
docker buildx imagetools inspect ghcr.io/rolealpha/good-workshop:0.4.1 --format '{{ json .SBOM }}'
```
